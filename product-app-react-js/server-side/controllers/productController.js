...
class ProductController {
    ...
    static async readProductById(req, res) {
        try {
            ...
            const product = await Product.findByPk(id, { include: ['Category', 'Images'] })
            if (!product) {
                throw { name: 'notFound' }
            }
    ...
    static async addProduct(req, res) {
        ...
        try {
            let { name, description, price, mainImg, categoryId, additionalImage1, additionalImage2, additionalImage3 } = req.body

            let slug = name.split(' ').join('-')

            const product = await Product.create({ name, description, slug, price, mainImg, categoryId, authorId: req.user.id }, { transaction: t })
    ...
    static async editProduct(req, res) {
        ...
        try {
            ...
            const { name, description, price, mainImg, categoryId, additionalImage1, additionalImage2, additionalImage3 } = req.body
            ...
            await Product.update({ name, description, slug, price, mainImg, categoryId }, { where: { id } }, { transaction: t })
